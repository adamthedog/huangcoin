...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/huangcoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in huangcoind / RPC client
            std::string strUsage = _("Huangcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  huangcoind [options]                     " + "\n" +
                  "  huangcoind [options] <command> [params]  " + _("Send command to server or huangcoind") + "\n" +
                  "  huangcoind [options] help                " + _("List commands") + "\n" +
                  "  huangcoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "huangcoin:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("Huangcoin"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("Huangcoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: huangcoin.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: huangcoind.pid)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the Huangcoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // huangcoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // huangcoin: checkpoint master priv key
    {
    ...
    // Make sure only a single Huangcoin process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Huangcoin is probably already running."), pszDataDir));
    ...
    //
    if (fDaemon)
        fprintf(stdout, "Huangcoin server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("Huangcoin"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Huangcoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Huangcoin to complete") << "\n";
